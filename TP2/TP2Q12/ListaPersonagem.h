...
void insercaoShellshort(ListaPersonagem* l, int cor, int h, int* c, int* m){
   for(int i = (cor+h); i < (*l).fim ; i+=h ){
      Personagem tmp = (*l).personagensArray[i];                 
      int j = i - h;

      /*Condiconal para verificar se peso do array eh maior que o da tmp, caso nao, verificar se
       * criteirio de desempate nome*/
       while((j >= 0) && ((*l).personagensArray[j].peso > tmp.peso ||((*l).personagensArray[j].peso 
             == tmp.peso && strcmp((*l).personagensArray[j].nome,tmp.nome )>0)) ){  
         (*c)++;
         (*m)++;
         (*l).personagensArray[j+h] = (*l).personagensArray[j];
         j = j - h;
      }       
      (*c)++; 
      (*m)++;
      (*l).personagensArray[j+h] = tmp;
   }
} 
/*Metodo para ordernar lista com shellshort*/
void shellshort(ListaPersonagem* l,int i, int* c, int* s){
   int h = 1;

   //repeticao para de encontrar h
   do{
      h = (h * 3) + 1;  
   }while(h < (*l).fim);
   ...
   do{
      ...
      for(int cor = 0; cor < h; cor++){
	      insercaoShellshort(l, cor, h, c, s);
      } 
   }while(h != 1);
}
