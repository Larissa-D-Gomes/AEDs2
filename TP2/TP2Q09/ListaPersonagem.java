...
class ListaPersonagem{
   ...
   /*Metodo para ordenar elementos na lista por meio de selecao*/
   public void selecao(){
      for(int i = 0; i < (this.fim-1);i++){
         int menor = i;
         for(int j = (i+1); j < this.fim; j++){
            this.numComparacoes++;
            if(this.personagensArray[j].getNome().compareTo(this.personagensArray[menor].getNome()) < 0){
               menor = j;
            }
         }
         swap(menor,i);
         this.numMovimentacoes+=3;
      }
   }
