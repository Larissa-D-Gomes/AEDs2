...
void inserirInicio(Lista* l, char arq[]){
...
void inserirFim(Lista* l, char arq[]){
   (*(*l).ultimo).prox = construtorCelula(arq);//Inserir nova celula no fim
...
void inserir(char arq[], int posicao, Lista* l){
   int t = tamanho(l);
   if(posicao > t || posicao < 0){//Teste para verificar se posicao eh valida 
      ...
      if(posicao == t){
         ...
         if(posicao == 0){
            ...
            Celula* tmp = construtorCelula(arq);//Criar nova celula
